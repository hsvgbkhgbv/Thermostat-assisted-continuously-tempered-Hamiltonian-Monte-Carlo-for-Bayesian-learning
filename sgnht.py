    ...
    def update(self):
        for name, module in self.model._modules.items():
            if self.pattern1.match(name):
                ...
                module.v_w.add_(- dw * module.eta_theta - module.z_theta * module.v_w + module.n_w.normal_() * (2 * module.c_theta * module.eta_theta / self.N).sqrt_())
                module.v_b.add_(- db * module.eta_theta - module.z_theta * module.v_b + module.n_b.normal_() * (2 * module.c_theta * module.eta_theta / self.N).sqrt_())
                w.add_(module.v_w)
                b.add_(module.v_b)
                module.z_theta.add_(((module.v_w**2).sum() + (module.v_b**2).sum()) / (w.numel() + b.numel()) - module.eta_theta / self.N)
            elif self.pattern2.match(name):
